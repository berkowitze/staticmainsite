...
function calculateHeights() {
  ...
  topHeight =
    about.height() +
    toNumb(about.css("padding-bottom")) +
    toNumb(about.css("margin-bottom"));
  aboutTrans = 0;
  resumeTrans = topHeight + 2 * toNumb(resume.css("margin-top"));
  projectsTrans =
    resumeTrans +
    resume.height() +
    2 * toNumb(projects.css("margin-top")) +
    toNumb(resume.css("margin-bottom")) +
    toNumb(resume.css("padding-bottom"));
  contactTrans =
    projectsTrans +
    projects.height() +
    2 * toNumb(contact.css("margin-top")) +
    toNumb(projects.css("margin-bottom")) +
    toNumb(resume.css("padding-bottom"));
}

function nameAndNavDone() {
  $("#name-div").css("margin-right", "");
  $("div.nav-buttons-transition")
    .removeClass("nav-buttons-transition")
    .addClass("nav-buttons-final sliding-u-l-r");
  ...
  $("#content-div").css(
    "width",
    ((docWidth - navWidth) / docWidth) * 100 - 5 + "%"
  );
  $("#content-div").addClass("content-div-after active-tab");
  $("#content-div").attr(
    "style",
    $("#content-div").attr("style") + " transform: translateX(0%) !important"
  );
  calculateHeights();
  setTimeout(function () {
    $(".About").addClass("active-tab-link");
  }, 200);
}
...
function openDiv(x) {
  ...
  if (!button.hasClass("active-tab-link")) {
    ...
    if (newDiv.hasClass("active")) {
      ...
      translate =
        " transform: translate(0%, " + -translateHeight + "px) !important";
...
function start(name) {
  ...
  if (TESTING) {
    ...
    setTimeout(function () {
      typeWord(name, nameDiv, 100, 40, initPage, 90);
    }, 100);
  }
}
